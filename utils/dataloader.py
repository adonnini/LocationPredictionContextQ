...
class sp_loc_dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if user is None:
        	print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - user is None")
        	self.is_individual_model = False
        else:
            self.is_individual_model = True
            print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - self.is_individual_model = True")
        ...
        if self.is_individual_model:
            ...
            print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - self.is_individual_model")
            ...
            print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - not self.is_individual_model")
...
#AD CHANGE
        if Path(save_path).is_file():
            ...
            print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - Path(save_path).is_file()")
            print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - save_path",save_path)
            print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - self.data",self.data)
        else:
        	print("LocationPrediction - utils - dataloader.py - sp_loc_dataset - NOT Path(save_path).is_file()")
    ...
    def __getitem__(self, idx):
        ...
        # [sequence_len] in a quarter of an hour
...
#AD - ADDED
        print("LocationPrediction - utils - dataloader.py - generate_data - 0 - ori_data - ",ori_data)
...
#AD - ADDED
        print("LocationPrediction - utils - dataloader.py - generate_data - 1 - ori_data - ",ori_data)

        # classify the datasets, user dependent 0.6, 0.2, 0.2
        train_data, vali_data, test_data = self._splitDataset(ori_data)
        print("LocationPrediction - utils - dataloader.py - generate_data - 0 - train_data - ",train_data)
        print("LocationPrediction - utils - dataloader.py - generate_data - 0 - vali_data - ",vali_data)
        print("LocationPrediction - utils - dataloader.py - generate_data - 0 - test_data - ",test_data)
        ...
        if self.is_individual_model:
            ...
            print("LocationPrediction - utils - dataloader.py - generate_data - 1 - train_data - ",train_data)
            print("LocationPrediction - utils - dataloader.py - generate_data - 1 - vali_data - ",vali_data)
            print("LocationPrediction - utils - dataloader.py - generate_data - 1 - test_data - ",test_data)
            print("LocationPrediction - utils - dataloader.py - generate_data - 1 - enc - ",enc)            
        ...
        print("LocationPrediction - utils - dataloader.py - generate_data - len(train_records) - ",len(train_records))
        print("LocationPrediction - utils - dataloader.py - generate_data - train_records - ",train_records)
        
        print("LocationPrediction - utils - dataloader.py - generate_data - len(validation_records) - ",len(validation_records))
        print("LocationPrediction - utils - dataloader.py - generate_data - validation_records - ",validation_records)
        
        print("LocationPrediction - utils - dataloader.py - generate_data - len(test_records) - ",len(test_records))
        print("LocationPrediction - utils - dataloader.py - generate_data - test_records - ",test_records)
        
        print("LocationPrediction - utils - dataloader.py - generate_data - self.data_type - ",self.data_type)
...
#    TEST10
        train_split = maxDay * 0.1
        vali_split = maxDay * 0.9
#    TEST9
#    train_split = maxDay * 0.3
#    validation_split = maxDay * 0.9
#    TEST8
#    train_split = maxDay * 0.4
#    validation_split = maxDay * 0.9
#    TEST7
#    train_split = maxDay * 0.5
#    validation_split = maxDay * 0.9
#    TEST6
#    train_split = maxDay * 0.2
#    validation_split = maxDay * 0.9
#    TEST5
#    train_split = maxDay * 0.5
...
#AD - ADDED
        print("LocationPrediction - utils - dataloader.py - _get_split_days_user - maxDay - ",maxDay)
        print("LocationPrediction - utils - dataloader.py - _get_split_days_user - train_split - ",train_split)
        print("LocationPrediction - utils - dataloader.py - - _get_split_days_user - vali_split - ",vali_split)
    ...
    def _preProcessDatasets(self, data, dataset_type):
        ...
        print("LocationPrediction - utils - dataloader.py - _preProcessDatasets - valid_records - ",valid_records)
...
#        return data
    ...
    def __getValidSequence(self, input_df):
        ...
        """
        print("LocationPrediction - utils - dataloader.py - __getValidSequence - input_df - ",input_df)
...
#AD - ADDED
        ...
        for index, row in df.iterrows():
            ...
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 0 --- ")
            
            # exclude the first records that do not include enough previous_day
#            if row["diff_day"] < self.previous_day:
#                continue

            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 1 --- ")
            # get the history records [curr-previous, curr]
            hist = df.iloc[:index]
            hist = hist.loc[(hist["start_day"] >= (row["start_day"] - self.previous_day))]
            
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 2 --- ")

            # should be in the valid user ids
#            if not (row["id"] in self.valid_ids):
#                continue

            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 3 --- ")
            ...
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 4 --- ")
            
            data_dict = {}
            # get the features: location, user, weekday, start time, duration, diff to curr day, and poi
            
            print("LocationPrediction - utils - dataloader.py - __getValidSequence - len(hist) - ",len(hist))
            if(len(hist) <= 0):
            	continue
            ...
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 5 --- ")

            if self.dataset == "gc":
                data_dict["poi_X"] = self._getPOIRepresentation(data_dict["X"])

            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 6 --- ")
            
            # the next location is the target
            data_dict["Y"] = int(row["location_id"])
            
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 7 --- ")

            # append the single sample to list
            data_single_user.append(data_dict)
            
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - STEP 8 --- ")
            
            print("LocationPrediction - utils - dataloader.py - ___getValidSequenceUser - data_dict - ",data_dict)
...
def test_dataloader(train_loader):
    ...
    for batch_idx, (x, y, x_dict) in tqdm(enumerate(train_loader)):
        # print("LocationPrediction - utils - dataloader.py - batch_idx ", batch_idx)
        x_shape += x.shape[0]
...
#    print("LocationPrediction - utils - dataloader.py - About to run dataset_train = sp_loc_dataset(... --- ")

#    dataset_train = sp_loc_dataset(source_root, dataset="geolife", data_type="train", previous_day=7)
#    print("LocationPrediction - utils - dataloader.py - if __name__ == __main__ - dataset_train - ",dataset_train)
