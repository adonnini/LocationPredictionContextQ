...
def single_run(train_loader, val_loader, test_loader, config, device, log_dir):
    ...
    # train
    print("LocationPrediction - main.py - single_run - About to run get_trainedNets --- ")
    model, perf = get_trainedNets(config, model, train_loader, val_loader, device, log_dir)
    print("LocationPrediction - main.py - single_run - model - ",model)
...
if __name__ == "__main__":
    ...
    config = edict(config)
    print("LocationPrediction - main.py - config - ",config)
